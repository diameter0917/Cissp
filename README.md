...
- **影片時間軸（P1 深連結）**：初期影片帶 `?t=秒數`「從約 mm:ss 開始」，連到主題**真正開講**處。`apply_video_timestamps.py` 用**密度定位**：跳過開場 120 秒的議程 name-drop，找關鍵字最密集的 120 秒視窗（多詞片語權重高於泛詞），回傳該段起點；定位不到才連開頭。時間碼來源為 `transcripts/_timed/<id>.vtt`（已 commit 8 支 course 影片）。重跑：`python scripts/apply_video_timestamps.py`。
