...
python scripts/apply_video_timestamps.py   # P1 影片補上 ?t=秒 深連結（需 transcripts/_timed/*.vtt，否則連開頭）
...
- **影片時間軸（P1 深連結）**：大綱要求初期影片帶 `?t=秒數`「從約 mm:ss 開始」。這需要 **帶時間碼的字幕** `transcripts/_timed/<id>.vtt`，但目前 repo 只 commit 了去時間碼的 `transcripts/*.txt`，所以 `apply_video_timestamps.py` 暫時讓所有 P1 影片連開頭、不標時間（優雅降級）。**要啟用時間碼**：在能連 YouTube 的本機跑 `prepare_youtube.py`（會產生 `transcripts/_timed/*.vtt`）、commit 進 repo，再跑 `python scripts/apply_video_timestamps.py`，每封 P1 信的影片連結就會自動帶上對應主題的起始秒數。機制已驗證可運作（含合成字幕測試）。
